...
function setTwo() {
	...
	while(!found) {
		...
		// Math.floor( - rounds downto the nearest integer)
		let r = Math.floor(Math.random() * rows);
		let c = Math.floor(Math.random() * columns);

		// Check if the positin (r,c) in the gameboard is empty
		if(board[r][c] == 0) {
			board[r][c] = 2;
...
// prevents scrolling if touvh input is received
...
// listen for the "touched" event on the entire document
